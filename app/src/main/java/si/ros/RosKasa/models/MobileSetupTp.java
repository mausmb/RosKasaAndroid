package si.ros.RosKasa.models;

public class MobileSetupTp {
    private int mobileId;
    private int tipkePosId = 512200;
    private Integer fPoslovniProstorId;
    private Integer fPosId;
    private Integer kuhinjaId;
    private Integer tocilnicaId;
    private int hisObrat = 512200;
    private int hisDestinacija;
    private Integer fiskalizacija;
    private String firma;
    private String naziv;
    private String nazivPodjetja;
    private String naslovPodjetja;
    private String nazivProdajnegaMesta;
    private String naslovProdajnega;
    private String obratProdajnegaMesta;
    private String printerRacuni;
    private String davcnaZaFurs;
    private String ddvPodjetja;
    private String nfcPrijava;
    private String vnosPogrinjkov;
    private String htColor;
    private String htStyleName;
    private String popustIzpis;
    private String popustLojalnost;
    private String popust99;
    private String kKarticeVPlacilih;
    private String nazivStregelVasJe;
    private String nazivZahvala1;
    private String nazivZahvala2;
    private String nazivZahvala3;
    private String nazivZahvala4;
    private String mobIni0;
    private String mobIni;
    private String mobIni2;
    private String steviloZnakov;
    private String escAlignCenter;
    private String escAlignLeft;
    private String escAlignRight;
    private String escBoldOff;
    private String escBoldOn;
    private String escCpi16;
    private String escCpi20;
    private String escCut;
    private String escEol;
    private String escInitPrint;
    private String escInverseOff;
    private String escInverseOn;
    private String escNewLine;
    private String escReset;
    private String escUnderlineOff;
    private String escUnderlineOn;
    private String escWidth2xOff;
    private String escWidth2xOn;
    private java.util.List<Integer> mobileSetupPlacila = new java.util.ArrayList<>();
    private java.util.List<OsebaTp> osebe = new java.util.ArrayList<>();
    private java.util.List<PrioritetaProjektaTp> prioriteteProjektov = new java.util.ArrayList<>();
    private java.util.List<TarifaTp> tarife = new java.util.ArrayList<>();
    private java.util.List<MizaTp> mobileSetupMize = new java.util.ArrayList<>();
    private java.util.List<Integer> rajoniList = new java.util.ArrayList<>();

    public MobileSetupTp() {}

    public int getMobileId() { return mobileId; }
    public void setMobileId(int mobileId) { this.mobileId = mobileId; }

    public int getTipkePosId() { return tipkePosId; }
    public void setTipkePosId(int tipkePosId) { this.tipkePosId = tipkePosId; }

    public Integer getfPoslovniProstorId() { return fPoslovniProstorId; }
    public void setfPoslovniProstorId(Integer fPoslovniProstorId) { this.fPoslovniProstorId = fPoslovniProstorId; }

    public Integer getfPosId() { return fPosId; }
    public void setfPosId(Integer fPosId) { this.fPosId = fPosId; }

    public Integer getKuhinjaId() { return kuhinjaId; }
    public void setKuhinjaId(Integer kuhinjaId) { this.kuhinjaId = kuhinjaId; }

    public Integer getTocilnicaId() { return tocilnicaId; }
    public void setTocilnicaId(Integer tocilnicaId) { this.tocilnicaId = tocilnicaId; }

    public int getHisObrat() { return hisObrat; }
    public void setHisObrat(int hisObrat) { this.hisObrat = hisObrat; }

    public int getHisDestinacija() { return hisDestinacija; }
    public void setHisDestinacija(int hisDestinacija) { this.hisDestinacija = hisDestinacija; }

    public Integer getFiskalizacija() { return fiskalizacija; }
    public void setFiskalizacija(Integer fiskalizacija) { this.fiskalizacija = fiskalizacija; }

    public String getFirma() { return firma; }
    public void setFirma(String firma) { this.firma = firma; }

    public String getNaziv() { return naziv; }
    public void setNaziv(String naziv) { this.naziv = naziv; }

    public String getNazivPodjetja() { return nazivPodjetja; }
    public void setNazivPodjetja(String nazivPodjetja) { this.nazivPodjetja = nazivPodjetja; }

    public String getNaslovPodjetja() { return naslovPodjetja; }
    public void setNaslovPodjetja(String naslovPodjetja) { this.naslovPodjetja = naslovPodjetja; }

    public String getNazivProdajnegaMesta() { return nazivProdajnegaMesta; }
    public void setNazivProdajnegaMesta(String nazivProdajnegaMesta) { this.nazivProdajnegaMesta = nazivProdajnegaMesta; }

    public String getNaslovProdajnega() { return naslovProdajnega; }
    public void setNaslovProdajnega(String naslovProdajnega) { this.naslovProdajnega = naslovProdajnega; }

    public String getObratProdajnegaMesta() { return obratProdajnegaMesta; }
    public void setObratProdajnegaMesta(String obratProdajnegaMesta) { this.obratProdajnegaMesta = obratProdajnegaMesta; }

    public String getPrinterRacuni() { return printerRacuni; }
    public void setPrinterRacuni(String printerRacuni) { this.printerRacuni = printerRacuni; }

    public String getDavcnaZaFurs() { return davcnaZaFurs; }
    public void setDavcnaZaFurs(String davcnaZaFurs) { this.davcnaZaFurs = davcnaZaFurs; }

    public String getDdvPodjetja() { return ddvPodjetja; }
    public void setDdvPodjetja(String ddvPodjetja) { this.ddvPodjetja = ddvPodjetja; }

    public String getNfcPrijava() { return nfcPrijava; }
    public void setNfcPrijava(String nfcPrijava) { this.nfcPrijava = nfcPrijava; }

    public String getVnosPogrinjkov() { return vnosPogrinjkov; }
    public void setVnosPogrinjkov(String vnosPogrinjkov) { this.vnosPogrinjkov = vnosPogrinjkov; }

    public String getHtColor() { return htColor; }
    public void setHtColor(String htColor) { this.htColor = htColor; }

    public String getHtStyleName() { return htStyleName; }
    public void setHtStyleName(String htStyleName) { this.htStyleName = htStyleName; }

    public String getPopustIzpis() { return popustIzpis; }
    public void setPopustIzpis(String popustIzpis) { this.popustIzpis = popustIzpis; }

    public String getPopustLojalnost() { return popustLojalnost; }
    public void setPopustLojalnost(String popustLojalnost) { this.popustLojalnost = popustLojalnost; }

    public String getPopust99() { return popust99; }
    public void setPopust99(String popust99) { this.popust99 = popust99; }

    public String getkKarticeVPlacilih() { return kKarticeVPlacilih; }
    public void setkKarticeVPlacilih(String kKarticeVPlacilih) { this.kKarticeVPlacilih = kKarticeVPlacilih; }

    public String getNazivStregelVasJe() { return nazivStregelVasJe; }
    public void setNazivStregelVasJe(String nazivStregelVasJe) { this.nazivStregelVasJe = nazivStregelVasJe; }

    public String getNazivZahvala1() { return nazivZahvala1; }
    public void setNazivZahvala1(String nazivZahvala1) { this.nazivZahvala1 = nazivZahvala1; }

    public String getNazivZahvala2() { return nazivZahvala2; }
    public void setNazivZahvala2(String nazivZahvala2) { this.nazivZahvala2 = nazivZahvala2; }

    public String getNazivZahvala3() { return nazivZahvala3; }
    public void setNazivZahvala3(String nazivZahvala3) { this.nazivZahvala3 = nazivZahvala3; }

    public String getNazivZahvala4() { return nazivZahvala4; }
    public void setNazivZahvala4(String nazivZahvala4) { this.nazivZahvala4 = nazivZahvala4; }

    public String getMobIni0() { return mobIni0; }
    public void setMobIni0(String mobIni0) { this.mobIni0 = mobIni0; }

    public String getMobIni() { return mobIni; }
    public void setMobIni(String mobIni) { this.mobIni = mobIni; }

    public String getMobIni2() { return mobIni2; }
    public void setMobIni2(String mobIni2) { this.mobIni2 = mobIni2; }

    public String getSteviloZnakov() { return steviloZnakov; }
    public void setSteviloZnakov(String steviloZnakov) { this.steviloZnakov = steviloZnakov; }

    public String getEscAlignCenter() { return escAlignCenter; }
    public void setEscAlignCenter(String escAlignCenter) { this.escAlignCenter = escAlignCenter; }

    public String getEscAlignLeft() { return escAlignLeft; }
    public void setEscAlignLeft(String escAlignLeft) { this.escAlignLeft = escAlignLeft; }

    public String getEscAlignRight() { return escAlignRight; }
    public void setEscAlignRight(String escAlignRight) { this.escAlignRight = escAlignRight; }

    public String getEscBoldOff() { return escBoldOff; }
    public void setEscBoldOff(String escBoldOff) { this.escBoldOff = escBoldOff; }

    public String getEscBoldOn() { return escBoldOn; }
    public void setEscBoldOn(String escBoldOn) { this.escBoldOn = escBoldOn; }

    public String getEscCpi16() { return escCpi16; }
    public void setEscCpi16(String escCpi16) { this.escCpi16 = escCpi16; }

    public String getEscCpi20() { return escCpi20; }
    public void setEscCpi20(String escCpi20) { this.escCpi20 = escCpi20; }

    public String getEscCut() { return escCut; }
    public void setEscCut(String escCut) { this.escCut = escCut; }

    public String getEscEol() { return escEol; }
    public void setEscEol(String escEol) { this.escEol = escEol; }

    public String getEscInitPrint() { return escInitPrint; }
    public void setEscInitPrint(String escInitPrint) { this.escInitPrint = escInitPrint; }

    public String getEscInverseOff() { return escInverseOff; }
    public void setEscInverseOff(String escInverseOff) { this.escInverseOff = escInverseOff; }

    public String getEscInverseOn() { return escInverseOn; }
    public void setEscInverseOn(String escInverseOn) { this.escInverseOn = escInverseOn; }

    public String getEscNewLine() { return escNewLine; }
    public void setEscNewLine(String escNewLine) { this.escNewLine = escNewLine; }

    public String getEscReset() { return escReset; }
    public void setEscReset(String escReset) { this.escReset = escReset; }

    public String getEscUnderlineOff() { return escUnderlineOff; }
    public void setEscUnderlineOff(String escUnderlineOff) { this.escUnderlineOff = escUnderlineOff; }

    public String getEscUnderlineOn() { return escUnderlineOn; }
    public void setEscUnderlineOn(String escUnderlineOn) { this.escUnderlineOn = escUnderlineOn; }

    public String getEscWidth2xOff() { return escWidth2xOff; }
    public void setEscWidth2xOff(String escWidth2xOff) { this.escWidth2xOff = escWidth2xOff; }

    public String getEscWidth2xOn() { return escWidth2xOn; }
    public void setEscWidth2xOn(String escWidth2xOn) { this.escWidth2xOn = escWidth2xOn; }

    public java.util.List<Integer> getMobileSetupPlacila() { return mobileSetupPlacila; }
    public void setMobileSetupPlacila(java.util.List<Integer> list) {
        this.mobileSetupPlacila = list != null ? list : new java.util.ArrayList<>();
    }

    public java.util.List<OsebaTp> getOsebe() { return osebe; }
    public void setOsebe(java.util.List<OsebaTp> list) {
        this.osebe = list != null ? list : new java.util.ArrayList<>();
    }

    public java.util.List<PrioritetaProjektaTp> getPrioriteteProjektov() { return prioriteteProjektov; }
    public void setPrioriteteProjektov(java.util.List<PrioritetaProjektaTp> list) {
        this.prioriteteProjektov = list != null ? list : new java.util.ArrayList<>();
    }

    public java.util.List<TarifaTp> getTarife() { return tarife; }
    public void setTarife(java.util.List<TarifaTp> list) {
        this.tarife = list != null ? list : new java.util.ArrayList<>();
    }

    public java.util.List<MizaTp> getMobileSetupMize() { return mobileSetupMize; }
    public void setMobileSetupMize(java.util.List<MizaTp> list) {
        this.mobileSetupMize = list != null ? list : new java.util.ArrayList<>();
        posodobiRajone();
    }

    public java.util.List<Integer> getRajoniList() { return rajoniList; }
    public void setRajoniList(java.util.List<Integer> list) {
        this.rajoniList = list != null ? list : new java.util.ArrayList<>();
    }

    public void posodobiRajone() {
        java.util.Set<Integer> unique = new java.util.TreeSet<>();
        if (mobileSetupMize != null) {
            for (MizaTp m : mobileSetupMize) {
                if (m.getRajon() != null && m.getRajon() > 0) {
                    unique.add(m.getRajon());
                }
            }
        }
        this.rajoniList = new java.util.ArrayList<>(unique);
    }
}
